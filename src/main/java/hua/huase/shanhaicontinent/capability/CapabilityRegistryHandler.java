...
import hua.huase.shanhaicontinent.entity.seamentitymob.SeaMEntityMobBase;
...
//    实体增加Capabilities事件
    ...
    public static void onAttachCapabilities(AttachCapabilitiesEvent event)
    {
        ...
        if (event.getObject() instanceof IMob&&event.getObject()instanceof Entity){
            ...
            if(event.getObject() instanceof SeaMEntityMobBase){
                SeaMEntityMobBase seaMEntityMobBase = (SeaMEntityMobBase)event.getObject();
                monsterCapabilityProvider = seaMEntityMobBase.addCapability(entity,random);
            }else{
    ...
    public static void onPlayerJoin(EntityJoinWorldEvent event){
        ...
        if (!entity.world.isRemote )
        {
            if (entity instanceof EntityPlayer) {
                ...
                MonsterCapability monsterCapability = null;
                try {
                     monsterCapability = entity.getCapability(CapabilityRegistryHandler.MONSTER_CAPABILITY, null);
        ...
        if (entity.world.isRemote ){
            if ((entity instanceof IMob && entity instanceof EntityLivingBase)||entity instanceof HunhuanEntity) {
                ...
                if(monsterCapability1!=null){
                    ...
                    ExampleMod.monsterHashMapCapability.remove(entity.getEntityId());

//                    if(entity instanceof SeaMEntityMobBase){
//                        entity.width *= (1f+Math.log10(monsterCapability.getNianxian())*0.5f);
//                        entity.height *= (1f+Math.log10(monsterCapability.getNianxian())*0.5f);
//
...
//        PacketHandler.INSTANCE.sendToAllTracking(new PacketPlayerCapability(newPlayerCapobility,event.getEntityPlayer()),new NetworkRegistry.TargetPoint(event.getEntityPlayer().dimension,event.getEntityPlayer().posX,event.getEntityPlayer().posY,event.getEntityPlayer().posZ,60));
    ...
    public static void onPlayerLogin(net.minecraftforge.fml.common.gameevent.PlayerEvent.PlayerLoggedInEvent event) {
        ...
        ItemStack itemStack = new ItemStack(HanderAny.wanfajieshao);
...
//                {
//                    ((ArmorApi) from.getItem()).removeAttributeModifiers(from,entity,entityequipmentslot);
////                    from.setCount(0);
//                }
//
//            }
