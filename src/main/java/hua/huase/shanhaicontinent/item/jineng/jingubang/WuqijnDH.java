...
public class WuqijnDH extends Item implements JinengMethond
{
    public WuqijnDH(String name, CreativeTabs Tabs)
    {
        super();
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        if(heldItem.getTagCompound()!=null&&heldItem.getTagCompound().getInteger("nianxian")>0){
            playerIn.getCooldownTracker().setCooldown(this, (int) (160-Math.log10(heldItem.getTagCompound().getInteger("nianxian"))*20));
        }else {
            return ActionResult.newResult(EnumActionResult.SUCCESS, heldItem);
        }
//        playerIn.setActiveHand(handIn);
        if(!worldIn.isRemote){
            List<Entity> list = worldIn.getEntitiesWithinAABBExcludingEntity(playerIn, playerIn.getEntityBoundingBox().grow(5.0D,0.0D,5.0D));
            for (Entity entity : list) {
                if (!worldIn.isRemote&&entity!=null&&entity instanceof EntityLivingBase && playerIn!=null && entity !=playerIn)
                {
                    entity.attackEntityFrom(DamageSource.causePlayerDamage(playerIn),playerIn.getCapability(CapabilityRegistryHandler.PLYAER_CAPABILITY,null).getWugong()*1.6f);
...
//添加数据
    ...
    public void addInformation(ItemStack itemstack, World world, List<String> list, ITooltipFlag flag) {
        ...
        list.add(net.minecraft.util.text.translation.I18n.translateToLocal("item.jingubang.jineng.dh.list0"));
        list.add(net.minecraft.util.text.translation.I18n.translateToLocal("item.jingubang.jineng.dh.list1"));
        if(itemstack.getTagCompound()!=null&&itemstack.getTagCompound().getInteger("nianxian")>0){
            ...
            list.add(net.minecraft.util.text.translation.I18n.translateToLocal("item.lingque.cd")
                    +""
                    + new BigDecimal(8-Math.log10(integer)).setScale(2, BigDecimal.ROUND_HALF_UP).floatValue()
                    +"s"
            );
        }
