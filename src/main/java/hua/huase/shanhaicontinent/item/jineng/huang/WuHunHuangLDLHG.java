...
public class WuHunHuangLDLHG extends Item implements JinengMethond
{
    public WuHunHuangLDLHG(String name, CreativeTabs Tabs)
    {
        super();
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        playerIn.getCooldownTracker().ticks+=20000;
        if(heldItem.getTagCompound()!=null&&heldItem.getTagCompound().getInteger("nianxian")>0){
            playerIn.getCooldownTracker().setCooldown(this, (int) (320-Math.log10(heldItem.getTagCompound().getInteger("nianxian"))*40));
        }else {
            return ActionResult.newResult(EnumActionResult.SUCCESS, heldItem);
        }
...
//添加数据
    ...
    public void addInformation(ItemStack itemstack, World world, List<String> list, ITooltipFlag flag) {
        ...
        list.add(net.minecraft.util.text.translation.I18n.translateToLocal("item.wuhun.huang.ldlhg.list0"));
        list.add(net.minecraft.util.text.translation.I18n.translateToLocal("item.wuhun.huang.ldlhg.list1"));
        if(itemstack.getTagCompound()!=null&&itemstack.getTagCompound().getInteger("nianxian")>0){
            ...
            list.add(net.minecraft.util.text.translation.I18n.translateToLocal("item.lingque.cd")
                    +""
                    + new BigDecimal(16-Math.log10(integer)*2).setScale(2, BigDecimal.ROUND_HALF_UP).floatValue()
                    +"s"
            );
        }
