...
public class WuqijnJGZ extends Item implements JinengMethond
{
    public WuqijnJGZ(String name, CreativeTabs Tabs)
    {
        super();
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        if(heldItem.getTagCompound()!=null&&heldItem.getTagCompound().getInteger("nianxian")>0){
            playerIn.getCooldownTracker().setCooldown(this, (int) (400-Math.log10(heldItem.getTagCompound().getInteger("nianxian"))*50));
        }else {
            return ActionResult.newResult(EnumActionResult.SUCCESS, heldItem);
        }

        if(!worldIn.isRemote){
          List<Entity> list = worldIn.getEntitiesWithinAABBExcludingEntity(playerIn, playerIn.getEntityBoundingBox().grow(8.0D,0.0D,8.0D));
            for (Entity entity : list) {
                if (!worldIn.isRemote&&entity!=null&&entity instanceof EntityLivingBase && playerIn!=null && entity !=playerIn)
                {
                    ((EntityLivingBase)entity).addPotionEffect(new PotionEffect(PotionRegistryHandler.Potion_Wuqijn_JGZ,160,0,true,true));
...
//添加数据
    ...
    public void addInformation(ItemStack itemstack, World world, List<String> list, ITooltipFlag flag) {
        ...
        list.add(net.minecraft.util.text.translation.I18n.translateToLocal("item.jingubang.jineng.jgz.list0"));
        list.add(net.minecraft.util.text.translation.I18n.translateToLocal("item.jingubang.jineng.jgz.list1"));
        if(itemstack.getTagCompound()!=null&&itemstack.getTagCompound().getInteger("nianxian")>0){
            ...
            list.add(net.minecraft.util.text.translation.I18n.translateToLocal("item.lingque.cd")
                    +""
                    + new BigDecimal(20-Math.log10(integer)/2*5).setScale(2, BigDecimal.ROUND_HALF_UP).floatValue()
                    +"s"
            );
        }
