...
public class WuqijnHtcLTLC extends Item implements JinengMethond
{
    public WuqijnHtcLTLC(String name, CreativeTabs Tabs)
    {
        super();
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        if(heldItem.getTagCompound()!=null&&heldItem.getTagCompound().getInteger("nianxian")>0){
            playerIn.getCooldownTracker().setCooldown(this, (int) (800-Math.log10(heldItem.getTagCompound().getInteger("nianxian"))*66));
        }else {
            return ActionResult.newResult(EnumActionResult.SUCCESS, heldItem);
        }
...
//添加数据
    ...
    public void addInformation(ItemStack itemstack, World world, List<String> list, ITooltipFlag flag) {
        ...
        list.add(net.minecraft.util.text.translation.I18n.translateToLocal("item.haotianchui.jineng.ltlc.list0"));
        list.add(net.minecraft.util.text.translation.I18n.translateToLocal("item.haotianchui.jineng.ltlc.list1"));
        if(itemstack.getTagCompound()!=null&&itemstack.getTagCompound().getInteger("nianxian")>0){
            ...
            list.add(net.minecraft.util.text.translation.I18n.translateToLocal("item.lingque.cd")
                    +""
                    + new BigDecimal(40-Math.log10(integer)*3.34).setScale(2, BigDecimal.ROUND_HALF_UP).floatValue()
                    +"s"
            );
        }
