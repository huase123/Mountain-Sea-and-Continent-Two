...
public class Lianyaohu  extends Item {
    ...
    public boolean itemInteractionForEntity(ItemStack stackold, EntityPlayer player, EntityLivingBase target, EnumHand hand) {
        ...
        if (!player.world.isRemote) {
            PlayerCapability capability = player.getCapability(PLYAER_CAPABILITY, null);
            MonsterCapability capability1 = target.getCapability(MONSTER_CAPABILITY, null);
            int nianxian = 0;
            if(capability1 != null){
                nianxian = capability1.getNianxian();
            }
            double pow = Math.pow(4, Math.log10(nianxian))+1;
            if(capability!=null && capability.getJingshenli()>=pow){
                capability.addJingshenli(-(float) pow);
                ...
                boolean b = player.addItemStackToInventory(stack);
                ...
                SendTitle.sendTitle((EntityPlayerMP)player,ACTIONBAR,RED,"message.lianyaohu.fail", (float) pow);
            }
