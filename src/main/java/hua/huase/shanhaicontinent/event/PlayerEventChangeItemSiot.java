...
//装备
        ...
        {
            ((ArmorApi) from.getItem()).removeAttributeModifiers(from,entityLiving,slot);
        }

        if (!to.isEmpty()&&to.getItem()instanceof ArmorApi   )
        {
            ((ArmorApi) to.getItem()).addAttributeModifiers(to,entityLiving,slot);
        }
