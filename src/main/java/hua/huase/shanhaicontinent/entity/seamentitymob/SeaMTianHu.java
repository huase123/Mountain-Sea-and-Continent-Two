...
import static net.minecraft.init.Biomes.ICE_MOUNTAINS;
import static net.minecraft.init.Biomes.ICE_PLAINS;

public class SeaMTianHu extends EntityMob implements SeaMEntityMobBase {



    public static final String ID = "SeaM_TianHu";
//    生成的群系，PLAINS平原  SAVANNA 热带草原    SAVANNA_PLATEAU 热带高原  MUTATED_PLAINS向日葵平原
    public static final Biome[] BIOMES = new Biome[] {ICE_PLAINS,ICE_MOUNTAINS};
    ...
    public static final EntityEntry TianHu =
            EntityEntryBuilder.create()
                    .entity(SeaMTianHu.class)
                    .id(SeaMTianHu.ID, EntityRegistryHandler.entityID++)
                    .name(ExampleMod.MODID + "." + SeaMTianHu.ID)
                    .tracker(80, 3, true)
                    .egg(0xe52222, 0xd7ef39)
                    .spawn(EnumCreatureType.MONSTER, 500, 4, 4, SeaMTianHu.BIOMES)
                    .build();

    public SeaMTianHu(World worldIn)
    {
        super(worldIn);
    ...
    private static final ResourceLocation LOOT_TABLE = LootTableList.register(
            new ResourceLocation(ExampleMod.MODID + ":entities/tianhu"));
...
//用于servce与client相互同步参数
    private static final DataParameter<Byte> COLOR =
            EntityDataManager.createKey(SeaMTianHu.class, DataSerializers.BYTE);
    private static final DataParameter<NBTTagCompound> LIVINGENTITYCAPABILITY =
            EntityDataManager.createKey(SeaMTianHu.class, DataSerializers.COMPOUND_TAG);
