...
public  class GuiPlayerExpanded extends GuiContainer
{
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.buttonList.add(new ChangeButton(2, this.guiLeft + 277 +22, this.guiTop + 10,20,10,">>"));
		this.buttonList.add(new CloseButton(3, this.guiLeft + 384, this.guiTop + -2,20,10,"X"));
	...
	protected void drawGuiContainerBackgroundLayer(float p_146976_1_, int mouseX, int mouseY) {
		...
		drawText("wuhun.display.name",left+277,top+12);
		if(monsterCapabilityList==null){
			drawText("wuhun.display.close",left+277,top+32);
			return;
		}
//		drawItemStack(new ItemStack(wuqijingubang),left+277+20,top+10,"");
		drawText(("wuhun.display.open." + capability.getWuhunListsname().get(capability.getHunhuankaiguan() - 1)),left+277,top+32);
		for (int i = 0; i < Math.min(monsterCapabilityList.size(),4); i++) {
			MonsterCapability monsterCapability = monsterCapabilityList.get(i);
			drawHunhuan(monsterCapability.getNianxian(),left+277,top+52+i*40,x,y);
			drawNianxian(monsterCapability.getNianxian(),left+277,top+52+i*40,x,y);
		}
		for (int i = 4; i < monsterCapabilityList.size(); i++) {

			MonsterCapability monsterCapability = monsterCapabilityList.get(i);
			drawHunhuan(monsterCapability.getNianxian(),left+332,top+52+(i-5)*40,x,y);
			drawNianxian(monsterCapability.getNianxian(),left+332,top+52+(i-5)*40,x,y);
		}
	...
	protected void drawText(String text,int left,int top) {
		...
		fontRenderer.drawString(net.minecraft.util.text.translation.I18n.translateToLocal(text),left, top, 0xff1a1104,false);
		GlStateManager.popMatrix();
	}
	...
	@SideOnly(Side.CLIENT)
	static class ChangeButton extends GuiButton {
		public ChangeButton(int buttonId, int x, int y, int widthIn, int heightIn, String buttonText) {
	...
	static class CloseButton extends GuiButton {
		public CloseButton(int buttonId, int x, int y, int widthIn, int heightIn, String buttonText) {
			super(buttonId, x, y, widthIn, heightIn, buttonText);
		}
